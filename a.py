...
def extract(source_list, extract_start, extract_length):
    ls = list(source_list)
    res = ls[extract_start:extract_length]
    return res
udf3 = udf(extract, ArrayType(StringType()))
impossible_negative = df_4.withColumn('extract_source', udf3(f.col("source_list"),f.col('extract_start'), f.col('extract_length')))
...
possible_negative = df4.withColumn('extract_source', udf3(f.col("source_list"),f.col('extract_start'), f.col('extract_length')))
